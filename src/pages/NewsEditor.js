...
function TiptapField({
  ...
  // Sync external value into editor (important for edit mode)
  useEffect(() => {
    if (!editor) return;
  ...
  return (
    <div className="mb-4">
      ...
      <div className="d-flex flex-wrap gap-2 mb-2 align-items-center">
        <button type="button" className="btn btn-sm btn-outline-secondary" aria-pressed={pressed("bold")} onClick={run(() => editor.chain().toggleBold())}>Bold</button>
        <button type="button" className="btn btn-sm btn-outline-secondary" aria-pressed={pressed("italic")} onClick={run(() => editor.chain().toggleItalic())}>Italic</button>
        <button type="button" className="btn btn-sm btn-outline-secondary" aria-pressed={pressed("strike")} onClick={run(() => editor.chain().toggleStrike())}>Strike</button>
        <button type="button" className="btn btn-sm btn-outline-secondary" aria-pressed={pressed("heading", { level: 1 })} onClick={run(() => editor.chain().toggleHeading({ level: 1 }))}>H1</button>
        <button type="button" className="btn btn-sm btn-outline-secondary" aria-pressed={pressed("heading", { level: 2 })} onClick={run(() => editor.chain().toggleHeading({ level: 2 }))}>H2</button>
        <button type="button" className="btn btn-sm btn-outline-secondary" aria-pressed={pressed("bulletList")} onClick={run(() => editor.chain().toggleBulletList())}>• List</button>
        <button type="button" className="btn btn-sm btn-outline-secondary" aria-pressed={pressed("orderedList")} onClick={run(() => editor.chain().toggleOrderedList())}>1. List</button>
        ...
        {/* Upload from device */}
        <button type="button" className="btn btn-sm btn-brand-primary" onClick={openFilePicker} disabled={isUploading} title="Upload image">
          {isUploading ? "Uploading…" : "Upload Image"}
        </button>
        <input ref={fileInputRef} type="file" accept="image/*" style={{ display: "none" }} onChange={onFileChange} />

        <button type="button" className="btn btn-sm btn-outline-secondary ms-auto" onClick={() => editor && editor.chain().focus().clearContent().run()}>
          Clear
        </button>
      </div>
      ...
      {uploadError && <div className="text-danger mt-2 small">⚠️ {uploadError}</div>}

      {tooShort && <div className="invalid-feedback d-block">Content must be at least {minChars} characters long</div>}
      <small className="text-muted">Tip: You can also paste or drag-drop an image to upload automatically.</small>
    </div>
  );
}
...
function NewsEditor() {
  ...
  // message modal + redirect countdown
  ...
  const safeNavigate = (path) => {
    if (hasUnsavedChanges) setConfirmLeave({ show: true, targetHref: path });
    else navigate(path);
  };
  ...
  // load edit mode
  useEffect(() => {
    const storedData = localStorage.getItem("editNewsData");
    if (storedData) {
      try {
        ...
        setValue("tags", newsData.tags ? newsData.tags.join(", ") : "");
        setValue("seoTitle", newsData.seoTitle || "");
        setValue("seoDescription", newsData.seoDescription || "");
        setContent(newsData.content || "");
        setValue("content", newsData.content || ""); // <-- keep RHF in sync
  ...
  useEffect(() => {
    const onDocClick = (e) => {
      // Allow opening in a new tab/window or with modifier keys
      ...
      // Ignore in-page anchors and non-navigation schemes
      const isInPage =
        url.hash &&
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Always append SEO fields (even if blank) so updates persist
      formData.append("seoTitle", (data.seoTitle ?? "").trim());
      formData.append("seoDescription", (data.seoDescription ?? "").trim());

      if (isEditMode && editData) {
        await axios.put(`${normalizeJoin(getApiBase(), "/api/news")}/${editData.id}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        showModal("success", "News Updated", "✅ Your news item has been updated successfully!");
      } else {
        await axios.post(normalizeJoin(getApiBase(), "/api/news"), formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        // success + 5s redirect
        ...
        showModal(
          ...
          "News Created",
          <>
            <div className="mb-2">✅ Your news item has been created successfully!</div>
            <div>Redirecting to Manage News in <strong>{5}</strong> seconds…</div>
          </>
        );
      }
      ...
      showModal(
        ...
        isEditMode
          ? "❌ There was a problem updating your news. Please try again."
          : "❌ There was a problem saving your news. Please try again."
      );
      setIsSubmitting(false);
    }
  };
  ...
  const handleCancel = () => {
    if (hasUnsavedChanges) {
      setConfirmLeave({ show: true, targetHref: "/newslist" });
    } else {
      reset();
      setImage(null);
      setIsEditMode(false);
  ...
  return (
    <>
              ...
              <div className="card-body p-4">
                <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <div className="mb-3">
                    ...
                    <input
                      type="text"
                      className={`form-control admin-form-control ${errors.tags ? "is-invalid" : ""}`}
                  ...
                  <div className="d-flex gap-2">
                    ...
                    >
                      {isSubmitting ? (
                        <>
                          ...
                          {isEditMode ? "Updating News..." : "Saving News..."}
                        </>
                      ) : isEditMode ? (
                        "Update News"
                      ) : (
                    ...
                    {isEditMode && (
                      <>
                        <button type="button" className="btn btn-lg btn-outline-secondary" onClick={handleCancel}>
                          Cancel
                        </button>
                        <button
                          type="button"
                          className="btn btn-lg btn-outline-brand"
                          onClick={() => safeNavigate("/newslist")}
                        >
                          ← Back to Manage
                        </button>
                      </>
        ...
        <div className="modal-overlay">
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button type="button" className="btn-close" onClick={() => setShowSEOModal(false)}></button>
            </div>
            <div className="modal-body">
              {/* SEO Title */}
              <div className="mb-3">
                ...
                <input
                  type="text"
                  className={`form-control admin-form-control ${errors.seoTitle ? "is-invalid" : ""}`}
                  placeholder="e.g. Breaking News: Latest Updates"
                  {...register("seoTitle", {
                    maxLength: { value: 60, message: "SEO title must be less than 60 characters" },
                    pattern: {
                      value: /^[a-zA-Z0-9\s\-_.,!?()&:;"'–—…]+$/,
                ...
                />
                {errors.seoTitle && <div className="invalid-feedback d-block">{errors.seoTitle.message}</div>}
                <small className="text-muted">Leave empty to use the main title. Keep under 60 characters.</small>
              </div>
              ...
              <div className="mb-3">
                ...
                <textarea
                  className={`form-control admin-form-control ${errors.seoDescription ? "is-invalid" : ""}`}
                  rows="3"
                  placeholder="e.g. Stay updated with the latest news and developments."
                  {...register("seoDescription", {
                    maxLength: { value: 160, message: "Meta description must be less than 160 characters" },
                    pattern: {
                      value: /^[a-zA-Z0-9\s\-_.,!?()&:;"'–—…]+$/,
                ...
                {errors.seoDescription && (
                  <div className="invalid-feedback d-block">{errors.seoDescription.message}</div>
                )}
                <small className="text-muted">Leave empty to auto-generate from content. Keep under 160 characters.</small>
              </div>
            </div>
            <div className="modal-footer">
              ...
              <button type="button" className="btn btn-secondary" onClick={() => setShowSEOModal(false)}>
                Close
              </button>
            </div>
          </div>
        ...
        message={
          redirectCountdown > 0 && modal.type === "success" ? (
            <>
              <div className="mb-2">✅ Your news item has been created successfully!</div>
              <div className="mb-2">
                Redirecting to Manage News in <strong>{redirectCountdown}</strong> seconds…
              </div>
              <button
        ...
        <div className="modal-overlay" onClick={() => setImgModal({ open: false, src: "" })}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "900px" }}>
            <div className="modal-header">
              ...
              <button type="button" className="btn-close" onClick={() => setImgModal({ open: false, src: "" })}></button>
            </div>
            <div className="modal-body">
              <img
                src={imgModal.src}
      ...
      {/* CONFIRM LEAVE MODAL */}
      <ConfirmLeaveModal show={confirmLeave.show} onStay={stayOnPage} onLeave={leavePage} />
    </>
  );
}
